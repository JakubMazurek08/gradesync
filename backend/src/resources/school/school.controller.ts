...
schoolController.get("/timetable/:id", authenticationMiddleware, async (req: Request, res: Response) => {
    ...
    try{
        const timetableResult = await dbClient.query(`SELECT timetable FROM schools WHERE id = $1`, [requestedId]);
        const daysResult = await dbClient.query(`
            SELECT
        ...
        const assignmentsResult = await dbClient.query(`SELECT 
          a.title, 
